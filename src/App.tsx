...
function ThreeDModelShowcase() {
  ...
  return (
    <section 
      id="models" 
      className="col-span-1 md:col-span-12 bg-zinc-900 border border-zinc-800 rounded-3xl p-6 md:p-8 flex flex-col relative overflow-hidden group select-none"
    >
      ...
      {/* Module Title - Clickable header to toggle collapse (exactly matches Videos button style) */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="flex justify-between items-center w-full focus:outline-none group/hdr"
      >
        <div className="flex items-center gap-3">
          <h3 className="font-bold uppercase text-xs tracking-[0.2em] text-zinc-500 group-hover/hdr:text-zinc-300 transition-colors">3D 課堂作業展示</h3>
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 font-mono">1 INTERACTIVE</span>
        </div>
        <motion.div
          animate={{ rotate: isOpen ? 180 : 0 }}
          transition={{ duration: 0.3 }}
          className="p-1.5 rounded-full bg-zinc-800/50 text-zinc-400 group-hover/hdr:bg-zinc-800 group-hover/hdr:text-zinc-300 transition-colors"
        >
          <ChevronDown size={16} />
        </motion.div>
      </button>

      {/* Main Container Layout with AnimatePresence (exactly matches Videos transition style) */}
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="pt-6">
              <div className="w-full flex flex-col md:flex-row items-center justify-center gap-8 py-3 select-text">
                <a
                  href={modelUrl}
                  target="_blank"
